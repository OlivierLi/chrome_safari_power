...
def RunScenario(scenario_config):
    ...
    if scenario_config.browser is not None:
        browser_executable = utils.BROWSERS_DEFINITION[
            scenario_config.browser]['executable']
        if scenario_config.browser in ["Chromium", "Chrome", "Canary", "Edge"]:
            subprocess.call(
                ["open", "-a", browser_executable, "--args"] +
    ...
    if scenario_config.browser:
        browser_process_name = utils.BROWSERS_DEFINITION[
            scenario_config.browser]['process_name']
    ...
    if scenario_config.background_script is not None:
        subprocess.call(
            ["osascript", f'./driver_scripts/{background_script}.scpt'])
...
def Record(scenario_config, output_dir):
    with open(
            f'./{output_dir}/{scenario_config.scenario_name}_powermetrics.plist',
            "w") as f:
...
def Profile(scenario_config, output_dir, dry_run, profile_mode):
    ...
    browser_process = FindBrowserProcess(
        utils.BROWSERS_DEFINITION[scenario_config.browser]['process_name'])
        ...
        while script_process.poll() is None:
            ...
            for pid in GetAllPids(browser_process):
                ...
                args = [
                    ...
                    f"{output_dir}/{pid}.txt", '-n', probe_def
                ]
...
class ScenarioConfig:
    ...
    def __init__(self, scenario_name, driver_script, browser, extra_args,
                 background_script):
        ...
        if browser == "Chromium" and "executable" not in utils.BROWSERS_DEFINITION[
                "Chromium"].keys():
            print(
                "Cannot run a Chromium scenario without the executable defined")
            exit(-1)
...
def main():
    ...
    parser.add_argument(
        ...
        help=
        "Run a profiling of the application in one of two modes: wakeups, cpu_time."
    )
    parser.add_argument(
        '--dry_run',
        dest='dry_run',
    ...
    if args.chromium_executable:
        utils.BROWSERS_DEFINITION["Chromium"][
    ...
    if args.run_measure:
        ...
        if args.meet_meeting_id:
            Record(
                ScenarioConfig("chrome_meet",
                               "chrome_meet",
                               browser="Chrome",
                               extra_args=chrome_extra_args,
                               background_script=None), args.output_dir)
            Record(
                ScenarioConfig("safari_meet",
                               "safari_meet",
                               browser="Safari",
                               extra_args=None,
                               background_script=None), args.output_dir)
            ...
            Record(
                ScenarioConfig("chrome_navigation",
                               "chrome_navigation",
                               browser="Chrome",
                               extra_args=chrome_extra_args,
                               background_script=None), args.output_dir)
            Record(
                ScenarioConfig("safari_navigation",
                               "safari_navigation",
                               browser="Safari",
                               extra_args=None,
                               background_script=None), args.output_dir)
    ...
    if args.profile_mode:
        Profile(ScenarioConfig("chromium_navigation",
                               "chromium_navigation",
                               browser="Chromium",
                               extra_args=[],
