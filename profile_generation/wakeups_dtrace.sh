...
cat ./samples/samples.collapsed.filtered | ./Flamegraph/flamegraph.pl --colors wakeup -w 3000 > ./samples/wakeups_flames_filtered.svg
open -a "Google Chrome" ./samples/wakeups_flames_filtered.svg

python3 filter.py --mode clean_only --stack_file samples/samples.collapsed > samples/samples.collapsed.cleaned
cat ./samples/samples.collapsed.cleaned | ./Flamegraph/flamegraph.pl --colors wakeup -w 3000 > ./samples/wakeups_flames_cleaned.svg
open -a "Google Chrome" ./samples/wakeups_flames_cleaned.svg
